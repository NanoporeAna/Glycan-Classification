...
if __name__ == '__main__':
    ...
    with pd.ExcelWriter(r'3ML_result.xlsx', mode='a', engine='openpyxl') as writer:
        ...
        df.to_excel(writer, sheet_name='2500randomForest', index=False)
        df1 = pd.DataFrame(confusion_matrix)
        df1.to_excel(writer, sheet_name='2500random_confusion_matrix', header=False, index=False)
        #     # 输出混淆矩阵
        #     confusion_matrix += metrics.confusion_matrix(test_labels, test_predict)
        # print('--混淆矩阵--')
        # print(confusion_matrix )
        ...
        # plt.savefig('../picture/mole 3 randomForest best.png')
        # plt.show()
