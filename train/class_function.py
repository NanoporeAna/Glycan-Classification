...
if __name__ == '__main__':
    ...
    # LogisticRegression, random_forest, support_vector_machine, AdaBoost, naive_bayes
    condition = 'LogisticRegression'  # 算法开关
    sample_type = 'windows'
    # 列出指定目录下的文件夹路径
    path = '../data/T240R二糖-NNR-Windows/featureBy3on2000' + sample_type
    file_name = [os.path.join(path, f) for f in os.listdir(path)]
    nmr_name = [f.strip('-feature.csv') for f in os.listdir(path)]  # 使用的数据名称
    mol_num = len(file_name)

    # 结果保存
    save_path = '../' + sample_type + date + 'result.xlsx'
    ...
    elif condition == "random_forest":
        model = RandomForestClassifier(n_estimators=80)  # 容易区分则设置为25
    elif condition == "LogisticRegression":
    ...
    for k in range(10):
        ...
        for i in range(epoch):
            ...
            joblib.dump(model, "../models/"+condition+date+sample_type+'.m')
            time_3 = time.time()
    ...
    print('混淆矩阵*')
    print(confusion_matrix)
    polt_conf(confusion_matrix, nmr_name, "../picture/" + condition + date +sample_type +'.png')
    ...
    with pd.ExcelWriter(save_path, mode='a', engine='openpyxl') as writer:
        ...
        df1.to_excel(writer, sheet_name=condition+'_confusion_matrix', header=False, index=False)
